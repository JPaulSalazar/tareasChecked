...
// VISTA - check para ver tareas (DOM).
const checkboxView = document.getElementById('hide-completed');
...
// VISTA - apendTaskDOM(): Agrega una nueva tarea al DOM.
checkboxView.addEventListener('click', (event) =>{
  const checkStatus = event.currentTarget.checked;
  const show = tareas.filter((tarea) => {
